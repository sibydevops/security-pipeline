...
function verifySignature(rawBody, signature) {
  ...
  const expected = crypto.createHmac('sha256', requiredEnvironment('GITHUB_WEBHOOK_SECRET'))
...
async function createInstallationToken(installationId) {
  ...
  const appJwt = jwt.sign({ iat: now - 60, exp: now + 540, iss: requiredEnvironment('GITHUB_APP_ID') }, privateKey, { algorithm: 'RS256' });
  const response = await fetch(`https://api.github.com/app/installations/${installationId}/access_tokens`, {
    method: 'POST',
    headers: { Accept: 'application/vnd.github+json', Authorization: `Bearer ${appJwt}`, 'X-GitHub-Api-Version': githubApiVersion }
  });
...
async function dispatchScan(eventName, payload) {
  ...
  const ref = isPullRequest ? `refs/pull/${payload.number}/merge` : payload.ref;
  ...
  const centralRepository = requiredEnvironment('CENTRAL_REPOSITORY');
  ...
  const response = await fetch(`https://api.github.com/repos/${centralRepository}/actions/workflows/${workflowId}/dispatches`, {
    method: 'POST',
    headers: { Accept: 'application/vnd.github+json', Authorization: `Bearer ${installationToken.token}`, 'X-GitHub-Api-Version': githubApiVersion, 'Content-Type': 'application/json' },
    body: JSON.stringify({ ref: workflowRef, inputs: { repository, ref, sha, event: eventName } })
  });
  if (!response.ok) throw new Error(`Workflow dispatch failed: ${response.status}`);
}

function registerGithubWebhook(app) {
  app.post('/github/webhook', express.raw({ type: 'application/json' }), async (request, response) => {
    if (!Buffer.isBuffer(request.body) || !verifySignature(request.body, request.get('x-hub-signature-256'))) {
